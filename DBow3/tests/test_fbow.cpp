...
class FastSearch{
 public:
  ...
  void create(Vocabulary &voc) {
    ...
    // where do children information start from the start of the block
    _child_off_start = _feature_off_start + voc.m_k * _desc_size_bytes;


    // block: nvalid|f0 f1 .. fn|ni0 ni1 ..nin
    _block_size_bytes = _feature_off_start +
        voc.m_k * (_desc_size_bytes + sizeof(node_info));
    ...
    for ( const auto &Block : parent_children ) {
      ...
      // 32 bits 100000000...0.check msb is not set
      assert(!(currblock & 0x80000000));
    ...
    for ( const auto &Block : parent_children ) {
      ...
      for ( const auto &c : Block.second ) {
        const auto &node = voc.m_nodes[nid_vpos[c]];
        node_info *ptr_child =
            (node_info*)(_data + block_offset_bytes
                         + _child_off_start + sizeof(node_info) * idx);
        ...
        if ( !node.isLeaf() ) {
          ...
          // set the msb to one to distinguish from offset
          ptr_child->id_or_childblock |= 0x80000000;
  ...
  node_info* getBestOfBlock(int block, const cv::Mat &desc) {
    ...
    int bestIdx = 0;
    for ( int i = 0; i < n; i++ ) {
      uchar  *ptr = (uchar*)(_data + block_start +
                             _feature_off_start + _desc_size_bytes * i);
      uint32_t d = 0;
    ...
    return (node_info*)(_data + block_start +
                        _child_off_start + sizeof(node_info) * bestIdx);
  }
  ...
  void transform(const cv::Mat &desc, DBoW3::BowVector &v) {
    for ( int i = 0; i < desc.rows; i++ ) {
      ...
      int wid;
      while ( !done ) {
        // =getBestOfBlock(block,desc.row(i));
        node_info *ni;
        {
          uint64_t block_start = block * _block_size_bytes;
          uint32_t mind = std::numeric_limits<uint32_t>::max();
          ...
          for ( i = 0; i < n4; i += 4 ) {
            uint64_t  *ptr =
                (uint64_t*)(_data + _toff + _desc_size_bytes * i);
            uint32_t d =
                __builtin_popcountl(dptr[0] ^ ptr[0]) +
            ...
            uint32_t d2 =
                __builtin_popcountl(dptr[0] ^ ptr_2[0]) +
            ...
            uint32_t d3 =
                __builtin_popcountl(dptr[0] ^ ptr_3[0]) +
            ...
            uint32_t d4 =
                __builtin_popcountl(dptr[0] ^ ptr_4[0]) +
