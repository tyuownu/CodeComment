...
class FastSearch {
 public:
  ...
  static cv::Mat features(DBoW3::Vocabulary &voc) {
    uint desc_size =
        DBoW3::DescManip::getDescSizeBytes(voc.m_words[0]->descriptor);
    cv::Mat feat(voc.m_words.size(), desc_size, CV_8UC1);

    for ( int i = 0; i < voc.m_words.size(); i++ ) {
      memcpy(feat.ptr<uchar>(i),
             voc.m_words[i]->descriptor.ptr<uchar>(0), desc_size);
...
int main(int argc, char **argv) {
  ...
  cv::flann::HierarchicalClusteringIndexParams
      indexParams(10, cvflann::FLANN_CENTERS_RANDOM, 4, 1) ;
