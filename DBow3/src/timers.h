    ...
    if ( use ) {
      ...
      std::cout << "Time (" << name << ")= " <<
          double(std::chrono::duration_cast<std::chrono::nanoseconds>(
                  end - begin).count()) / fact << str << std::endl;
    }
  }
};
...
struct ScopedTimerEvents {
  ...
  void addspaces(std::vector<std::string> &str) {
    ...
    for ( auto &s : str ) {
      while ( static_cast<int>(s.size()) < m )
        s.push_back(' ');
    }
  }
  ...
  ~ScopedTimerEvents() {
    ...
    for ( size_t i = 1; i < vtimes.size(); i++ ) {
        std::cout << "Time(" << _name << ")-" << names[i] << " "
            << double(std::chrono::duration_cast<std::chrono::nanoseconds>(
                    vtimes[i] - vtimes[i - 1]).count()) / fact << str << " "
            << double(std::chrono::duration_cast<std::chrono::nanoseconds>(
                    vtimes[i] - vtimes[0]).count()) / fact << str << std::endl;
    }
  }
};
...
struct Timer {
  ...
  void end() {
    ...
    sum += double(std::chrono::duration_cast<std::chrono::nanoseconds>(
            e-_s).count());
    n++;
  }
