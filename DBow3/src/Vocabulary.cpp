...
Vocabulary::Vocabulary(int k, int L,
...
//  DUtils::Random::SeedRandOnce();
  ...
  // DUtils::Random::RandomInt(0, pfeatures.size()-1);
...
void Vocabulary::transform(const std::vector<cv::Mat>& features,
  ...
  if ( m_weighting == TF || m_weighting == TF_IDF ) {
    ...
    for ( auto fit = features.begin();
         fit < features.end(); ++fit, ++i_feature) {
      WordId id;
      NodeId nid;
      WordValue w;
      // w is the idf value if TF_IDF, 1 if TF
...
void Vocabulary::load(const std::string &filename) {
  ...
  if ( !ifile )
    throw std::runtime_error("Vocabulary::load Could not open file :" +
...
void Vocabulary::toStream(std::ostream &out_str, bool compressed)
  ...
  if ( compressed ) {
    ...
    while ( total_size != 0 ) {
      ...
      uint64_t  compressed_size = qlz_compress(&input[0], &compressed[0],
                                               readSize, &state_compress);
      total_size -= readSize;
