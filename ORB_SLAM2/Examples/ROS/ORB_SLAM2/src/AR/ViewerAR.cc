...
void ViewerAR::Run() {
  ...
  pangolin::CreatePanel("menu")
      .SetBounds(0.0, 1.0, 0.0, pangolin::Attach::Pix(wui));
  ...
  pangolin::GlTexture imageTexture(w, h, GL_RGB, false,
...
void ViewerAR::SetImagePose(const cv::Mat &im, const cv::Mat &Tcw,
                            const int &status, const vector<cv::KeyPoint> &vKeys,
...
void ViewerAR::AddTextToImage(const string &s, cv::Mat &im, const int r,
  ...
  cv::putText(im, s, cv::Point(l, im.rows-l), cv::FONT_HERSHEY_PLAIN,
              1.5, cv::Scalar(255, 255, 255), 2, 8);
  cv::putText(im, s, cv::Point(l-1, im.rows-l), cv::FONT_HERSHEY_PLAIN,
              1.5, cv::Scalar(255, 255, 255), 2, 8);
  cv::putText(im, s, cv::Point(l+1, im.rows-l), cv::FONT_HERSHEY_PLAIN,
              1.5, cv::Scalar(255, 255, 255), 2, 8);
  cv::putText(im, s, cv::Point(l-1, im.rows-(l-1)), cv::FONT_HERSHEY_PLAIN,
              1.5, cv::Scalar(255, 255, 255), 2, 8);
...
void ViewerAR::DrawCube(const float &size, const float x,
...
Plane* ViewerAR::DetectPlane(const cv::Mat Tcw,
  ...
  for ( int n=0; n < iterations; n++ ) {
    ...
    for ( int i=0; i < N; i++ ) {
      vDistances[i] = fabs(vPoints[i].at<float>(0)*a+
                           vPoints[i].at<float>(1)*b+
                           vPoints[i].at<float>(2)*c+d)*f;
    }
...
Plane::Plane(const std::vector<MapPoint *> &vMPs,
             const cv::Mat &Tcw) :
    mvMPs(vMPs),
    mTcw(Tcw.clone()) {
