...
int main(int argc, char **argv)
{
  ...
  if ( argc != 4 ) {
    cerr << endl << "Usage: rosrun ORB_SLAM2 Stereo path_to_vocabulary"
        " path_to_settings do_rectify" << endl;
    ros::shutdown();
    return 1;
  }
  ...
  message_filters::Subscriber<sensor_msgs::Image>
      left_sub(nh, "/camera/left/image_raw", 1);
  message_filters::Subscriber<sensor_msgs::Image>
      right_sub(nh, "camera/right/image_raw", 1);
