...
bool LoopClosing::DetectLoop() {
  ...
  // If the map contains less than 10 KF or less than 10 KF
  // have passed from last loop detection
  ...
  // Each candidate expands a covisibility group (keyframes
  // connected to the loop candidate in the covisibility graph)
  ...
  for ( size_t i=0, iend=vpCandidateKFs.size(); i < iend; i++ ) {
    ...
    // If the group is not consistent with any previous group
    // insert with consistency counter set to zero
...
bool LoopClosing::ComputeSim3() {
  ...
  while ( nCandidates > 0 && !bMatch ) {
    for ( int i=0; i < nInitialCandidates; i++ ) {
      ...
      // If RANSAC returns a Sim3, perform a guided matching and
      // optimize with all correspondences
      if ( !Scm.empty() ) {
...
void LoopClosing::CorrectLoop() {
  ...
  // Retrive keyframes connected to the current keyframe and
  // compute corrected Sim3 pose by propagation
  ...
  Optimizer::OptimizeEssentialGraph(
      mpMap, mpMatchedKF, mpCurrentKF, NonCorrectedSim3,
...
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap) {
  ...
  for ( KeyFrameAndPose::const_iterator mit=CorrectedPosesMap.begin(),
    ...
    vector<MapPoint*> vpReplacePoints(
        mvpLoopMapPoints.size(), static_cast<MapPoint*>(NULL));
