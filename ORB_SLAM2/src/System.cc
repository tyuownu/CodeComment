...
System::System(const string &strVocFile, const string &strSettingsFile,
  ...
  // (it will live in the main thread of execution,
  // the one that called this constructor)
...
cv::Mat System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight,
  ...
  if ( mSensor != STEREO ) {
    cerr << "ERROR: you called TrackStereo but input sensor"
        " was not set to STEREO." << endl;
    exit(-1);
  }
...
void System::SaveTrajectoryTUM(const string &filename) {
  ...
  // We need to get first the keyframe pose and then concatenate
