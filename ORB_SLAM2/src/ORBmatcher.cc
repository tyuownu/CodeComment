...
int ORBmatcher::SearchByProjection(
    Frame &F, const vector<MapPoint*> &vpMapPoints, const float th) {
  int nmatches = 0;
  ...
  for ( size_t iMP=0; iMP < vpMapPoints.size(); iMP++ ) {
    ...
    // Apply ratio to second match (only if best and
    // second are in the same scale level)
    if ( bestDist <= TH_HIGH ) {
...
int ORBmatcher::SearchByBoW(KeyFrame* pKF, Frame &F,
  ...
  // We perform the matching over ORB that belong to
  // the same vocabulary node (at a certain level)
...
int ORBmatcher::SearchByProjection(
    KeyFrame* pKF, cv::Mat Scw, const vector<MapPoint*> &vpPoints,
...
int ORBmatcher::SearchForInitialization(
    Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched,
    ...
    if ( pMP ) {
      if ( !LastFrame.mvbOutlier[i] ) {
        ...
        if ( bForward )
            ...
            vIndices2 =
                CurrentFrame.GetFeaturesInArea(u, v, radius, nLastOctave-1,
                                               nLastOctave+1);
