...
int main(int argc, char **argv)
{
  try {
    ...
    if ( cml["-h"] || argc < 3 ) {
      cerr << "Usage:  features output.fbow [-k k] [-l L] [-t nthreads]"
          << " [-maxIters <int>:0 default] [-v verbose on]. " << endl;
      cerr << "Creates the vocabylary of k^L" << endl;
      cerr << "By default, we employ a random selection center without "
          "runnning a single iteration of the k means.\n"
          "As indicated by the authors of the flann library in their paper, the"
          " result is not very different from using k-means, but speed is much better\n";
      return -1;
    }
    ...
    cout << "time="
        << double(std::chrono::duration_cast<std::chrono::milliseconds>(
                t_end - t_start).count()) << " msecs" << endl;
