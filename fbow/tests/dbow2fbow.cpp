...
class VocabularyCreator {
 public:
  struct ninfo {
    ninfo() {}
    ninfo(uint32_t Block, ORBVocabulary::Node *Node) :
        block(Block),
        node(Node)
    {}
  ...
  static void convert(ORBVocabulary &voc, fbow::Vocabulary &out_voc) {
    ...
    for ( int i = 0; i < voc.m_nodes.size(); i++ ) {
      ...
      if ( !node.isLeaf() )
        nodeid_info.insert(
            std::make_pair(node.id, ninfo(nonLeafNodes++, &node)));
      else
        nodeid_info.insert(std::make_pair(node.id, ninfo(-1, &node)));
    }

    out_voc.setParams(8, voc.m_k, CV_8UC1, 32, nonLeafNodes, "orb");
    cerr << "creating     size=" << out_voc._params._total_size / (1024*1024)
        << "Mb " << out_voc._params._total_size << " bytes" << endl;
      ...
      if ( !node.isLeaf() ) {
        ...
        for ( int c = 0; c < node.children.size(); c++ ) {
          ...
          if ( child_info.node->isLeaf() ) {
            binfo.getBlockNodeInfo(c)->setLeaf(child_info.node->word_id,
                                               child_info.node->weight);
          } else {
...
{
  if ( argc != 3 ) {
    cerr << "Usage voc.txt out.fbow" << endl;
    return -1;
  }
