...
class FBOW_API VocabularyCreator {
 public:
  struct Params {
    Params() {}
    Params(uint32_t K, int l = -1, uint32_t Nthreads = 1, int MaxIters = -2) :
        k(K),
        L(l),
        nthreads(Nthreads) {
      if ( MaxIters != -2 )
  ...
  void create(fbow::Vocabulary &Voc, const std::vector<cv::Mat> &features,
              const std::string &desc_name, Params params);
  void create(fbow::Vocabulary &Voc, const cv::Mat &features,
              const std::string &desc_name, Params params);

 private:
  Params _params;
  struct feature_info {
    feature_info(uint32_t MIDX, uint32_t FIDX) :
        midx(MIDX),
        fidx(FIDX)
    {}

    // matrix feature into the vector of matrices and fidx feature
    // into the matrix
    uint32_t midx, fidx;
    float m_Dist;
    uint32_t parent = 0;
  ...
  struct FeatureInfo {
    ...
    inline cv::Mat operator[](uint32_t i) {
      const auto &idx = fInfo[i];
      return features->at(idx.midx).row(idx.fidx);
    }
  ...
  void createLevel(const std::vector<uint32_t> &findices,
                   int parent = 0, int curL = 0);
  void createLevel(int parent = 0, int curL = 0, bool recursive = true);
  std::vector<uint32_t> getInitialClusterCenters(
  ...
  // queue of parent to be processed
  Queue<std::pair<int, int> > ParentDepth_ProcesQueue;
  ...
  inline std::pair<uint32_t, uint32_t> separe(uint64_t a_b) {
    uint32_t *_a_b_16 = (uint32_t*)&a_b;
  ...
  struct Node {
    Node() {}
    Node(uint32_t Id, uint32_t Parent, const cv::Mat &Feature,
    ...
    // index of the feature this node represent(only if leaf and
    // it stop because not enough points to create a new leave.
    // In case the node is a terminal point, but has many points
    // beloging to its cluster, then, this is not set.
  ...
  void assignToClusters(const std::vector<uint32_t> &findices,
                        const std::vector<cv::Mat> &center_features,
                        std::vector<vector_sptr> &assigments, bool omp = false);
  std::vector<cv::Mat> recomputeCenters(
      const std::vector<vector_sptr> &assigments, bool omp = false);
  std::size_t vhash(const std::vector<vector_sptr>& v_vec);

  // std::map<uint64,std::vector<uint32_t> > id_assigments;
  ...
  // do not  know how to create dinamically :S
  std::atomic<bool> threadRunning[100];
