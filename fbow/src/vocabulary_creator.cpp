...
void VocabularyCreator::create(fbow::Vocabulary &Voc, const cv::Mat  &features,
                               const std::string &desc_name, Params params) {
  std::vector<cv::Mat> vfeatures(1);
  vfeatures[0] = features;
  create(Voc, vfeatures, desc_name, params);
}

void VocabularyCreator::create(fbow::Vocabulary &Voc,
                               const std::vector<cv::Mat> &features,
                               const string &desc_name, Params params) {
  ...
  if ( !(_descType == CV_8UC1 || _descType == CV_32FC1) )
    throw std::runtime_error(
        "Descriptors must be binary CV_8UC1 or float  CV_32FC1");
  ...
  if ( _params.nthreads > 1 ) {
    ...
    for ( size_t i = 0; i < _Threads.size(); i++ )
      ParentDepth_ProcesQueue.push(std::make_pair(-1, -1));
...
void VocabularyCreator::createLevel(int parent, int curL, bool recursive) {
  ...
  if ( findices.size() <= _params.k ) {
    ...
    for ( size_t i = 0; i < _params.k; i++ ) {
      id_assigments.create(parent * _params.k + 1 + i,
                           findices.size() / _params.k);
    ...
    while ( niters < _params.maxIters && cur_hash != prev_hash ) {
      ...
      // do assigment
      assignToClusters(findices, center_features,
                       assigments_ref /*,parent==0*/);
  ...
  new_nodes.reserve(center_features.size());
  {
    for ( size_t c = 0; c < center_features.size(); c++)
      new_nodes.push_back(
          Node(parent*_params.k + 1 + c, parent, center_features[c],
               findices.size() == center_features.size() ?
               findices[c] : std::numeric_limits<uint32_t>::max()));
  }
  ...
  // should we go deeper?
  if ( ((_params.L != -1 && curL < (_params.L-1)) || _params.L == -1)
    && assigments_ref.size() > 0 ) {
    ...
    if ( recursive ) {   // recursive mode(one thread only)
      ...
      for ( size_t i = 0; i < new_nodes.size(); i++)
        ParentDepth_ProcesQueue.push(
            std::make_pair(parent * _params.k + 1 + i, curL + 1));
    }
  }
}

std::vector<uint32_t> VocabularyCreator::getInitialClusterCenters(
    const std::vector<uint32_t> &findices) {
  // set distances to zero

  for ( auto fi : findices )
    _features(fi).m_Dist = 0;
...
std::size_t VocabularyCreator::vhash(
    const std::vector<std::vector<uint32_t> > &v_vec) {
  std::size_t seed = 0;

  for ( size_t i = 0; i < v_vec.size(); i++ )
    seed += v_vec[i].size() * (i + 1);
...
void VocabularyCreator::assignToClusters(
    const std::vector<uint32_t> &findices,
    const std::vector<cv::Mat> &center_features,
    std::vector<vector_sptr> &assigments, bool omp) {
  for ( auto &a : assigments )
    a->clear();

  if ( omp ) {
    std::vector<std::map<uint32_t, std::list<uint32_t> > >
        map_assigments_omp(omp_get_max_threads());
#pragma omp parallel for
    for ( int i = 0; i < int(findices.size()); i++ ) {
      ...
      std::pair<uint32_t, float>
          center_dist_min(0, dist_func(center_features[0], feature));

      for ( size_t ci = 1; ci < center_features.size(); ci++ ) {
        float dist = dist_func(center_features[ci], feature);
        if ( dist < center_dist_min.second )
          center_dist_min = std::make_pair(ci, dist);
      }
...
std::vector<cv::Mat> VocabularyCreator::recomputeCenters(
    const std::vector<vector_sptr> &assigments, bool omp) {
...
cv::Mat VocabularyCreator::meanValue_binary(
...
cv::Mat VocabularyCreator::meanValue_float(
    const std::vector<uint32_t> &indices) {
...
void VocabularyCreator::convertIntoVoc(Vocabulary &Voc, std::string desc_name) {
  ...
  for ( auto &node : TheTree.getNodes() ) {
    if ( node.second.isLeaf() ) {
      ...
      if ( node.second.feat_idx == std::numeric_limits<uint32_t>::max() )
        node.second.feat_idx = nLeafNodes;
  ...
  if ( _descType == CV_32F )
    aligment = 32;
  Voc.setParams(aligment, _params.k, _descType, _descNBytes,
                nonLeafNodes, desc_name);
...
float VocabularyCreator::distance_hamming_generic(
...
// for orb
float VocabularyCreator::distance_hamming_32bytes(
    const cv::Mat &a, const cv::Mat &b) {
  const uint64_t *pa = a.ptr<uint64_t>();    // a & b are actually CV_8U
  const uint64_t *pb = b.ptr<uint64_t>();
  return uint64_popcnt(pa[0] ^ pb[0]) + uint64_popcnt(pa[1] ^ pb[1]) +
      uint64_popcnt(pa[2] ^ pb[2]) + uint64_popcnt(pa[3] ^ pb[3]);
}

float VocabularyCreator::distance_float_generic(
    const cv::Mat &a, const cv::Mat &b) {
  double sqd = 0.;
