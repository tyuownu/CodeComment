...
class FBOW_API Vocabulary {
 ...
 private:
  ...
  // N: 16 bits : number of nodes in this block. Must be <=branching factor k.
  //    If N<k, then the block has empty spaces since block size is fixed
  ...
  // Fi: feature of the node i. it is aligned and padding added to the end
  //     so that F(i+1) is also aligned
  // CiWi: are the so called block_node_info (see structure up)
  // Ci: either if the node is leaf (msb is set to 1) or not. If not leaf,
  //     the remaining 31 bits is the block where its children are.
  //     Else, it is the index of the feature that it represent
  // Wi: float value empkoyed to know the weight of a leaf node
  //     (employed in cases of bagofwords)
  struct Block {
    Block(char* bsptr, uint64_t ds, uint64_t ds_wp, uint64_t fo, uint64_t co) :
        _blockstart(bsptr),
        _desc_size_bytes(ds),
        _desc_size_bytes_wp(ds_wp),
        _feature_off_start(fo),
        _child_off_start(co)
    {}
    ...
    inline void getFeature(int i, cv::Mat feature) {
      memcpy(feature.ptr<char>(0), _blockstart + _feature_off_start +
             i * _desc_size_bytes, _desc_size_bytes);
    }
    ...
    char *_blockstart;
    // size of the descriptor(without padding)
    uint64_t _desc_size_bytes = 0;
    // size of the descriptor(includding padding)
    uint64_t _desc_size_bytes_wp = 0;
    uint64_t _feature_off_start = 0;
    // into the block,where the children offset part starts
    uint64_t _child_off_start = 0;
  };
  ...
  inline Block getBlock(uint32_t b) {
    ...
    return Block(_data + b * _params._block_size_bytes_wp, _params._desc_size,
                 _params._desc_size_bytes_wp, _params._feature_off_start,
  ...
  class Lx {
   ...
   public:
    ...
    void setParams(int desc_size, int block_desc_size_bytes_wp) {
      ...
      _nwords = _block_desc_size_bytes_wp / sizeof(register_type);
      feature = (register_type*)AlignedAlloc(aligment,
                                             _nwords * sizeof(register_type ));
